...
int dataReceivedCallback(USB_Status_TypeDef status, uint32_t xferred, uint32_t remaining)
{
  ...
  // test if reset to bootloader
  else if( xferred == 6 && receiveBuffer[0] == 'g' && receiveBuffer[1] == 'o' &&
           receiveBuffer[2] == 'b' && receiveBuffer[3] == 'o' && receiveBuffer[4] == 'o' && receiveBuffer[5] == 't' ) {
    receiveBuffer[0] = 'G'; receiveBuffer[1] = 'O';
    shouldReboot = true;
  }
